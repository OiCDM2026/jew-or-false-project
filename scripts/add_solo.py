"""Append standalone statements (no paired sibling, pairId: null) to
statements.json, in either direction (true or false). Generalizes
add_solo_false.py to support mixed batches.

Usage: python3 scripts/add_solo.py scripts/<batch>.py
The batch module must define a top-level ENTRIES list of
(category, statement, isTrue, explanation) 4-tuples.
"""
import json
import sys
...
def main():
    if len(sys.argv) != 2:
        print("Usage: add_solo.py <path-to-batch-file>")
        sys.exit(1)
    ...
    for i, (category, statement, is_true, explanation) in enumerate(entries):
    ...
    print(f"Added {len(entries)} standalone statements.")
